...
- **SC-002**: The overdue visual treatment is distinct from both the normal
  ...
  distinguish all three states without instruction (verified visually against
...
- "Current date" is determined using UTC-based comparison
  (`new Date().toISOString().split('T')[0]`). Due dates stored by the backend
  use the same UTC-aligned ISO date format, ensuring consistent comparison.
...
- Timezone handling: the UTC-based comparison is the canonical approach for
  this feature; no additional timezone normalization is in scope.
...
- Q: What UTC-based date comparison approach should be used for overdue detection? → A: `new Date().toISOString().split('T')[0]` (UTC string comparison against stored ISO date strings; resolves timezone ambiguity noted in analysis finding I1)